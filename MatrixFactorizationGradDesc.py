...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    gradU_wo_Reg = joinedRDD.map(lambda (i, j, dij, ui, vj): (i, gradient_u(dij, ui, vj))) \
    ...
    U_joined = U.join(gradU_wo_Reg, numPartitions=N) # format (i, (ui, grad_ui_wo_Reg)
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    gradV_wo_Reg = joinedRDD.map(lambda (i, j, dij, ui, vj): (j, gradient_v(dij, ui, vj))) \
        .reduceByKey(lambda grad_ij, grad_ik: grad_ij + grad_ik, numPartitions=N)  # format of (j, sum_grad_vij = grad_vj_wo_Reg).
    # Note that regularization term is missing so we add it in next line

    V_joined = V.join(gradV_wo_Reg, numPartitions=N)  # format (j, (vj, grad_vj_wo_Reg)
...
if __name__=="__main__":
    ...
    test = sc.textFile(dataFolder + 'test').map(eval).cache()
    Mtrain = train.count()
    Mtest = test.count()

    # List for holding rmses
    cross_val_rmses = []
    train_rmses = []
    ...
    # We have finished the loop. So now we evaluate the final result
    now = time() - start
    testRMSE = np.sqrt(1. * SE(joinAndPredictAll(test, U, V, args.N)) / Mtest)
    trainMSE = np.sqrt(1. * SE(joinedRDD) / Mtrain)
    train_rmses.append(trainMSE)
